/* ==========================================================================
   Martinoz Franchise Intelligence Platform - Shared Mock Database Layer (data.js)
   Provides persistent database state in localStorage across all dashboard pages.
   ========================================================================== */
...
(function(window) {
  ...
  // --- Seed Data for Outlets (40 Outlets across cities in Gujarat) ---
  ...
  // --- Seed Data for Complaint CRM Tickets ---
  ...
  const DEFAULT_KDS = [
    ...
    { id: 'ORD-1002', outletId: 'OUT-05', customerName: ' नेहा Patel', items: '1x Garlic Breadsticks, 1x Lava Cake', status: 'Preparing', prepTimeMinutes: 8, timestamp: '2026-06-18T17:45:00Z' },
  ...
  function initDatabase() {
    ...
    if (!localStorage.getItem(seedFlag)) {
      localStorage.removeItem(DB_PREFIX + 'outlets');
      localStorage.removeItem(DB_PREFIX + 'tickets');
      localStorage.removeItem(DB_PREFIX + 'skus');
      localStorage.removeItem(DB_PREFIX + 'inventory');
      localStorage.removeItem(DB_PREFIX + 'kds');
      localStorage.removeItem(DB_PREFIX + 'iot');
      localStorage.removeItem(DB_PREFIX + 'shipments');
      localStorage.removeItem(DB_PREFIX + 'promos');
      localStorage.setItem(seedFlag, 'true');
    }
    if (!localStorage.getItem(DB_PREFIX + 'outlets')) {
      localStorage.setItem(DB_PREFIX + 'outlets', JSON.stringify(DEFAULT_OUTLETS));
    }
    if (!localStorage.getItem(DB_PREFIX + 'tickets')) {
      localStorage.setItem(DB_PREFIX + 'tickets', JSON.stringify(DEFAULT_TICKETS));
    }
    if (!localStorage.getItem(DB_PREFIX + 'skus')) {
      localStorage.setItem(DB_PREFIX + 'skus', JSON.stringify(DEFAULT_SKUS));
    }
    if (!localStorage.getItem(DB_PREFIX + 'inventory')) {
      localStorage.setItem(DB_PREFIX + 'inventory', JSON.stringify(DEFAULT_INVENTORY));
    }
    if (!localStorage.getItem(DB_PREFIX + 'kds')) {
      localStorage.setItem(DB_PREFIX + 'kds', JSON.stringify(DEFAULT_KDS));
    }
    if (!localStorage.getItem(DB_PREFIX + 'iot')) {
      localStorage.setItem(DB_PREFIX + 'iot', JSON.stringify(DEFAULT_IOT));
    }
    if (!localStorage.getItem(DB_PREFIX + 'shipments')) {
      localStorage.setItem(DB_PREFIX + 'shipments', JSON.stringify(DEFAULT_SHIPMENTS));
    }
    if (!localStorage.getItem(DB_PREFIX + 'promos')) {
      localStorage.setItem(DB_PREFIX + 'promos', JSON.stringify(DEFAULT_PROMOS));
    }
  }
  ...
  const Database = {
    getOutlets: function() {
      initDatabase();
      return JSON.parse(localStorage.getItem(DB_PREFIX + 'outlets'));
    },

    saveOutlets: function(outlets) {
      localStorage.setItem(DB_PREFIX + 'outlets', JSON.stringify(outlets));
    },

    getTickets: function() {
      initDatabase();
      return JSON.parse(localStorage.getItem(DB_PREFIX + 'tickets'));
    },

    saveTickets: function(tickets) {
      localStorage.setItem(DB_PREFIX + 'tickets', JSON.stringify(tickets));
    },
    ...
    resolveTicket: function(ticketId) {
      ...
      if (ticket) {
        ...
        // Decrement complaint count on corresponding outlet
    ...
    escalateTicket: function(ticketId) {
      const tickets = this.getTickets();
      const ticket = tickets.find(t => t.id === ticketId);
      if (ticket) {
    ...
    addTicket: function(ticketData) {
      ...
      // Increment complaint count on corresponding outlet
      if (targetOutlet) {
    ...
    submitHygieneAudit: function(outletId, score, answers) {
      const outlets = this.getOutlets();
      const outlet = outlets.find(o => o.id === outletId);
      if (outlet) {
        outlet.hygieneScore = score;
        this.recalculateOutletAlertStatus(outlet);
        this.saveOutlets(outlets);
      }
    },

    recalculateOutletAlertStatus: function(outlet) {
      if (outlet.hygieneScore < 70 || outlet.openComplaints >= 5) {
        outlet.alertStatus = 'critical';
      } else if (outlet.hygieneScore < 80 || outlet.openComplaints >= 3) {
        outlet.alertStatus = 'warning';
      } else {
        outlet.alertStatus = 'normal';
      }
    },
    ...
    getGlobalStats: function() {
      ...
      let sumHygiene = 0;
      let totalWastage = 0;
      let warningOutlets = 0;
      let criticalOutlets = 0;

      outlets.forEach(o => {
        totalRevenue += o.sales;
      ...
      const avgHygiene = outlets.length > 0 ? (sumHygiene / outlets.length) : 0;
      ...
      return {
        ...
        avgHygieneScore: avgHygieneScore,
        openComplaints: openTicketsCount,
        totalWastageKg: totalWastage,
        warningOutletsCount: warningOutlets,
        criticalOutletsCount: criticalOutlets,
    ...
    getSkus: function() {
      initDatabase();
      return JSON.parse(localStorage.getItem(DB_PREFIX + 'skus'));
    },

    getInventory: function() {
      initDatabase();
      return JSON.parse(localStorage.getItem(DB_PREFIX + 'inventory'));
    },

    saveInventory: function(inventory) {
      localStorage.setItem(DB_PREFIX + 'inventory', JSON.stringify(inventory));
    },
    ...
    getInventoryStats: function() {
      ...
      inventory.forEach(item => {
        ...
        if (diff < 0) {
          const lostSales = Math.abs(diff) * sku.price;
          totalLostSales += lostSales;
          if (Math.abs(diff) >= 20) {
            totalAlerts++;
          }
        } else if (diff > 0) {
          const wastage = diff * sku.price;
          totalWastageVal += wastage;
          if (diff >= 20) {
            totalAlerts++;
          }
        }
      ...
      const alignmentScore = inventory.length > 0 
        ? Math.round((alignedCount / inventory.length) * 100) 
        : 100;
    ...
    getKds: function() {
      initDatabase();
      return JSON.parse(localStorage.getItem(DB_PREFIX + 'kds'));
    },

    saveKds: function(kds) {
      localStorage.setItem(DB_PREFIX + 'kds', JSON.stringify(kds));
    },

    getIot: function() {
      initDatabase();
      return JSON.parse(localStorage.getItem(DB_PREFIX + 'iot'));
    },

    saveIot: function(iot) {
      localStorage.setItem(DB_PREFIX + 'iot', JSON.stringify(iot));
    },

    getShipments: function() {
      initDatabase();
      return JSON.parse(localStorage.getItem(DB_PREFIX + 'shipments'));
    },

    saveShipments: function(shipments) {
      localStorage.setItem(DB_PREFIX + 'shipments', JSON.stringify(shipments));
    },

    getPromos: function() {
      initDatabase();
      return JSON.parse(localStorage.getItem(DB_PREFIX + 'promos'));
    },

    savePromos: function(promos) {
      localStorage.setItem(DB_PREFIX + 'promos', JSON.stringify(promos));
    },

    addPromo: function(code, discount, description) {
      ...
      if (existing) {
        ...
        promos.push({ code: code.toUpperCase(), discount: discount, description: description, status: 'Active' });
      }
